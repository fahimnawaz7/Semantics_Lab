...
for i in range(len(lx)):
    ...
    if((x0,y0) in V_line_end_pt or (x0,y0) in H_line_end_pt): # If list of list write [x0, y0] 
        continue
    else:
        for j in range(len(V_line_end_pt)):
            x1 = int(ly[ly.index((x0, y0)) + j + 1][0]) 
            if ((x1,y0) in V_line_end_pt): # If list of list write [x1, y0] 
                continue
            else:
                break

        for k in range(len(H_line_end_pt)):      
            y1 = int(lx[lx.index((x0,y0)) + k + 1][1])
            if ((x0,y1) in H_line_end_pt):  # If list of list write [x0, y1]
                continue
            else:
                break
